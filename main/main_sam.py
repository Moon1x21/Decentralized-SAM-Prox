...
from workers.worker_vision_sam import *
from utils.scheduler import *
from utils.utils import *
from utils.samopt import SAM
...
def main(args):
    ...
    log_id = 'Deeplearning_' + args.dataset_name + '_sam_'+ args.mode 
    ...
    log_name = args.optimization+'_Scheduling'
    ...
    for rank in range(args.size):
        ...
        if args.amp:
            ...
            if args.optimization == "base":
                worker = Worker_Vision(model, rank, optimizer, scheduler, args.eta, arg.rho , train_loader, args.device, False)
            else:
                rho_list = np.clip(np.array([args.rho * round(pow(0.1,(1-int(i / (args.early_stop / 2)))),2) for i in range(args.early_stop)]),0,1)
                # import pdb; pdb.set_trace()
                worker = Worker_Vision(model, rank, optimizer, scheduler, args.eta, rho_list , train_loader, args.device, True)
        ...
        for _ in range(train_loader.__len__()):
            if args.mode == 'csgd':
                ...
                if((iteration % args.local_iter == 0)):
                    for worker in worker_list:
                        ...
                        worker.update_localprox() # update local_prox_model
            ...
            if iteration % 50 == 0:    
                ...
                if args.amp:
                    ...
                    wandb.log({
                            'rho': worker_list[0].sam_optimizer.rho
                        })               
                else:
